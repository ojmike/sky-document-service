...
EXPOSE 3400 3400
# Install and setup
ADD target/document-service-0.0.1-SNAPSHOT.jar document-service-0.0.1-SNAPSHOT-SNAPSHOT.jar
...
ENTRYPOINT java  -jar -Dspring.profiles.active=docker  -XX:-OmitStackTraceInFastThrow $JAVA_OPTS document-service-0.0.1-SNAPSHOT-SNAPSHOT.jar
